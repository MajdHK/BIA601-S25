...
def recommend(user_id: int):
    ...
    for pid, raw_sc in results:
        ...
        recs.append(OneProduct(
            product_id=int(pid), category=str(p["category"]), price=float(p["price"]),
            score=round(min(raw_sc / 3.2, 1.0), 2),
            rating=round(float(db.avg_ratings.get(pid, 0.0)), 1)
        ))
    return ResponseBody(user_id=int(user["user_id"]), user_location=str(user.get("location", "Unknown")), recommendations=recs)

@app.get("/users")
def get_users():
    return {"users": db.users_df.head(20)[["user_id", "location", "age"]].to_dict(orient="records")}
